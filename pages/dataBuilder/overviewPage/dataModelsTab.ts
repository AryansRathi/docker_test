...
export class DataModelsTab {
    ...
    readonly dataModelCard: (technicalName: string) => DataModelCard;
    ...
    constructor(parent: OverviewPage) {
        ...
        this.dataModelCard = (technicalName: string) => new DataModelCard(this, technicalName);
    }

    public async getDataModelCards(): Promise<DataModelCard[]> {
        const elements = await this.root
            .locator('div[data-testid*="DB-ModelsList-ListRow"].v-card')
            .all();
        const cards: DataModelCard[] = [];
        for (const element of elements) {
            const testId = await element.getAttribute('data-testid');
            const match = testId?.match(/^DB-ModelsList-ListRow-(.+)-IxOverviewCard-Card$/);
            if (match) {
                cards.push(new DataModelCard(this, match[1]));
            }
        }
        return cards;
    }
    ...
    public async importFromDatabase({
        ...
        if (tableNames) {
            ...
            await this.importFromDatabaseModal.importStep3SelectAllButton.click();
        }
        await this.importFromDatabaseModal.importStep3NextButton.click();

        await this.importFromDatabaseModal.technicalNameInput.fill(dataModel.technicalName);
        await this.importFromDatabaseModal.technicalNameInput.blur();
        await this.importFromDatabaseModal.nameInput.fill(dataModel.name);
        await this.importFromDatabaseModal.descriptionInput.fill(dataModel.description);
        await this.importFromDatabaseModal.importStep4FinishButton.click();
    }
    ...
    public async downloadExport(dataModel: TestErDataModel) {
        await this.dataModelCard(dataModel.technicalName).export();
