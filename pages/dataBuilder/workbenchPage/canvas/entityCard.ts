...
export class EntityCard {
    ...
    constructor(parent: Canvas, technicalName: string) {
        ...
        this.root = parent.root.getByTestId(`DB-EntityElement-Div-${technicalName}`);

        this.keyIcon = this.root.getByRole('img', { name: 'key' });
        this.connectionHandle = (attributeName: string, side: 'left' | 'right') =>
            this.root.getByTestId(`DB-EntityElement-Div-${attributeName}-${side}`);
    }
}
